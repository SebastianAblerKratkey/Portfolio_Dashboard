...
option = st.sidebar.selectbox("What do you want to see?", ("Past performance", "Retrun correlation", "MVP, ORP and OCP", "Minimum varriance frontier and Capital allocation line", "Savings plan simulation", "Data"))
...
if download_sucess:
    ...
    if option == "Past performance":
        ...
        display_summary["Mean Retrun p.a."] = summary["mean retrun"]
        ...
        display_summary["Mean Retrun p.a."] = display_summary["Mean Retrun p.a."].map('{:.2%}'.format)
    ...
    if option == "MVP, ORP and OCP":
        ...
        if weight_rf > 0:
            ...
            In this case the OCP would be expected to generate a retrun of {float(KPIs_ocp['portfolio retrun']):.2%} 
    ...
    if option == "Savings plan simulation":
        ...
        additional_investment_per_year = st.number_input("How much money do you want to save each year?",min_value=0.0, value=0.0)
        ...
        if selected_p == "MVP":
            sim_summary = mvp_summary_abrev
        elif selected_p == "ORP":
            sim_summary = orp_summary_abrev
        elif selected_p == "OCP":
            sim_summary = ocp_summary_abrev
        ...
        total_yearly_investments = num_years * additional_investment_per_year
        ...
        if st.button("Run simulation"):
            ...
            for index, row in sim_summary.iterrows():
                sim_list.append(np.random.normal(row["mean retrun"], row["standard deviation"], size=(num_trials, num_years)) * row["weight"])

            simulated_retruns = np.array(sim_list).sum(axis=0)

            # Calculate the potential future values of the investment
            val_future = np.zeros((num_trials, num_years+1))
            val_future[:,0] = val_today

            total_yearly_investments = 0
            for i in range(1, num_years+1):
                val_future[:,i] = val_future[:,i-1] * np.exp(simulated_retruns[:,i-1]) + additional_investment_per_year
                total_yearly_investments += additional_investment_per_year
            ...
            simulated_performance.set_index(pd.Index(future_years), inplace=True)

            # Calculate the average simulated performance across all trials
            avg_simulated_performance = simulated_performance.mean(axis=1)

            # Calculate the cumulative deposits
            if additional_investment_per_year > 0:
                cumulative_depositis = np.arange(val_today, 
                                    val_today+total_yearly_investments+additional_investment_per_year,
            ...
            cash_flow = [-val_today]+[-additional_investment_per_year]*(num_years-1)+[expected_capital-additional_investment_per_year]
            ...
            sim_text = (
                    ...
                    f"This corresponds to an expected time-weighted retrun of {avg_log_TWR:.2%} p.a. "
                    f"and an expected money-weighted retrun (IRR) of {irr:.2%} p.a."
                    )
            st.write(sim_text)
            ...
            while percent_in_interval <= 1.0:
                ...
                if round(percent_in_interval,5) == p:
                    break
            ...
            st.markdown(
                        ...
                        - {p:.0%} of simulation outcomes lie between {currency_formatter(expected_capital - moe, currency=currency)} and {currency_formatter(expected_capital + moe, currency=currency)}
